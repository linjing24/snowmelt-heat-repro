...
def plot_four_vars_from_folder(folder,
    ...
    text_pos = [
        ...
        {"x":1985,"y_factor":0.2, "x_offset":8,  "y_offset":30},
        {"x":1985,"y_factor":0.1, "x_offset":8,  "y_offset":3},
    ]
...
def main():
    ...
    folder = str(ROOT / "data" / "气象-年尺度-站点（7.9）" / "华北地区")   # <<< 改成读取的“包含多个站点文件”的文件夹
    output_dir = str(ROOT / "results" / "3.1-华北地区")          # <<< 输出目录


    # 四个面板的定义：列名、标题、颜色、单位、上限比例、y主刻度步长
    var_defs = [
        ('total_precipitation',  'Precipitation', 'orange',  'mm', 1.8, 300),
        ('total_rain',           'Rain',          'red',     'mm', 1.4, 300),
        ('total_snow',           'Snow',          'blue',    'mm', 1.5, 20),
    ...
    plot_four_vars_from_folder(
        ...
        output_dir=r"C:\Users\A\Desktop\3.1-华北地区"
    )
